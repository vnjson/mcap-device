...
export default function (){
  ...
  $tpl.find('.select-word__variants').on('click', '.select-word__item', function (e){
      ...
      $(this).parent()
             ...
             .toArray()
             .forEach(node=>$(node).removeClass('select-word__item--active'))
  ...
  $dropZone.on('click', ()=>{
        ...
        let answerItemHTML = $(`<div class="select-word__item">${_scriptData.answers.variants[indexItem]}</div>`)
...
function handler (scriptData){
  ...
  if(scriptData){
      ...
      answers.variants.forEach( (item, index)=>{
        const str = $(`<div class="select-word__item" data-index="${index}">${item}</div>`)
