(function () {
  ...
  const $tpl$7 = $(tpl$a);
  function dialogBox () {
    $tpl$7.css('background-image', `url(${dialogBoxImage})`);
    this.$store.$stream.append($tpl$7);
    let disabled = false;
    $tpl$7.on('mousedown', e => {
      if (!disabled) {
        this.next();
      }
    });
    this.on('character', (character, reply) => {
      if (character.avatar) {
        $tpl$7.find('.dialog-box__reply-wrapper').css('width', '75%');
        $tpl$7.find('.dialog-box__avatar').show().css({
          backgroundImage: `url('${this.getAssetByName(character.avatar).url}')`
        });
        $tpl$7.find('.dialog-box__name').html(character.name).css({
          color: character.nameColor
        });
        $tpl$7.find('.dialog-box__reply').html(reply).css({
          color: character.replyColor
        });
      } else {
        $tpl$7.find('.dialog-box__reply-wrapper').css('width', '90%');
        $tpl$7.find('.dialog-box__avatar').hide();
        $tpl$7.find('.dialog-box__name').html(character.name).css({
          color: character.nameColor
        });
        $tpl$7.find('.dialog-box__reply').html(reply).css({
    ...
    this.on('dialog-box', param => {
      if (param === 'clear') {
        $tpl$7.find('.dialog-box__name').empty();
        $tpl$7.find('.dialog-box__reply').empty();
        $tpl$7.find('.dialog-box__avatar').css('background-image', `unset`);
        disabled = false;
  ...
  $tpl$5.on("click", ".main-menu__item", clickHundler);
  ...
  function clickHundler() {
    ...
    $tpl$5.hide(); //$tpl.off( "click", clickHundler)
  }
  /**
   *  onClick:
   *    dialog-box: true
  ...
  var css$7 = ".stream__wiki {\n  left: 10px;\n  top: 40px;\n  width: 300px;\n  display: none;\n  height: auto;\n  padding: 10px;\n  border-radius: 4px;\n  background-color: rgba(0,0,0,0.5);\n  z-index: 7100;\n  font-size: 14px;\n}\n.stream__wiki li{\n  padding: 5px 0;\n\n  line-height: 18px;\n    \n}\n.stream__wiki-key{\n  color: wheat;\n}\n.stream__wiki-dash{\n  padding: 0 5px;\n  color: silver;\n}\n.stream__wiki-value{\n  color: darkgray;\n}";
  n(css$7,{});

  let $tpl$4 = $('<ul class="stream__wiki component"></ul>');
  function wiki () {
  ...
  let click = true;
  $tpl$3.find('.stream__test-variants').on('click', '.stream__variants-item', function () {
  ...
  $tpl$3.find('.stream__test-next-btn').on('click', function () {
    $vnjs.exec({
      next: true,
      test: false
    });
  ...
  var css$4 = ".screen-stream__slide{\n  position: absolute;\n  display: none;\n  top: 5%;\n  left: 50%;\n  transform: translateX(-50%);\n  box-shadow: 3px 3px 12px rgba(0, 0, 0, 0.5);\n  z-index: 2000;\n}";
  n(css$4,{});

  const $tpl$2 = $(`<img class="screen-stream__slide component"/>`);
  function slide () {
    ...
    this.on('slide', id => {
      ...
      if (typeof id === 'string') {
        $tpl$2.attr('src', this.getAssetByName(id).url).show();
        ...
        if (id.hasOwnProperty('css')) {
          $tpl$2.css(id.css);
          $tpl$2.attr('src', id.name).show();
        }

        if (id.hasOwnProperty('url')) {
          $tpl$2.attr('src', id.url).fadeIn();
        } else {
          $tpl$2.attr('src', this.getAssetByName(id.name).url).show();
        }
        /*
        .css(id.css)
         */
  ...
  const $tpl$1 = $(`<ul class="stream__stage component"></ul>`);
  function content () {
  ...
  function renderTree(data) {
    ...
    function clickHandler() {
      ...
      $tpl$1.off('click', clickHandler);
    }

    $tpl$1.on('click', '.stage-item', clickHandler);
  }
